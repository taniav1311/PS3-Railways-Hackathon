...
4.  Route recommendations and alternative paths
5.  Network vulnerability and resilience analysis
...
- When discussing tickets, always suggest alternatives if chances are low."""
        ...
        if any(word in query_lower for word in
            ...
            if self.delay_predictor:
                try:
                    ...
                    context_parts.append(f" Delay data error: {e}")
            ...
            if self.congestion_analyzer:
                try:
                    ...
                    for route_name, data in corridors.items():
                        context_parts.append(
                            f"\n {route_name} CORRIDOR CONGESTION:\n{data.to_string(index=False)}"
                        )
                    ...
                    context_parts.append(f" Congestion data error: {e}")
        ...
        if any(word in query_lower for word in
            ...
            if self.confirmation_predictor:
                try:
                    ...
                    context_parts.append(f" Booking data error: {e}")
        ...
        if any(word in query_lower for word in
            ...
            if self.railway_network:
                try:
                    ...
                    context_parts.append("\n ROUTES (Mumbai Central → New Delhi):")
                    ...
                    context_parts.append(f" Network data error: {e}")
    ...
    def _get_general_overview(self):
        ...
        if self.railway_network:
            stats = self.railway_network.get_network_stats()
            overview.append(f"\n NETWORK: {stats['total_stations']} stations, "
                          f"{stats['total_unique_trains']} trains, "
    ...
    def _chat_offline(self, query, context):
        ...
        # ---- Congestion queries ----
            ...
            if self.congestion_analyzer:
                ...
                hotspots = self.congestion_analyzer.get_hotspots(now.hour)
                if not hotspots.empty:
                    parts.append(f"\n **{len(hotspots)} Congestion Hotspots Active!**")
        ...
        # ---- Route queries ----
            ...
            if self.railway_network:
                parts.append("** Route & Network Analysis:**\n")
                ...
                centrality = self.railway_network.get_centrality_analysis()
                parts.append(f"\n **Most Critical Stations (if disrupted):**")
